...
export default function ResourcesPage({ initialResources }: { initialResources: Resource[] }) {
  ...
  useEffect(() => {
    ...
    if (saved === "light" || saved === "dark") {
      // The browser preference is read after hydration to avoid server/client markup drift.
  ...
  useEffect(() => {
    if (!search && !filterSubject && !filterClass && !filterBoard && !filterType) {
      return
    }

    // Show the existing results while the debounced filtered request is pending.
    // eslint-disable-next-line react-hooks/set-state-in-effect
    setLoading(true)
  ...
  }, [search, filterSubject, filterClass, filterBoard, filterType, initialResources])
